var debug = require('debug')('anomalyDetector-algorithm');
...
var AnomalyDetector = {
    ...
    train : function (random_variables, cb) {
        ...
        debug("distributions: "+JSON.stringify(distributions));
     ...
     *
     * @param {integer} d_id - Random distribution id
     * @param {integer} v_id - Random value id
     * @param {float} v - Random variable value
     * @returns {callable}
     * @access public
     */
    test : function (d_id,v_id, v, cb) {
        this.data_source.get(d_id, function(distribution){
            var E = distribution.e,
                sigma = distribution.sigma;

            var result = Math.abs(E - v) <= (3 * sigma);   // check if the value is VALID or NOT. True --> OK --- False --> not Ok (is outlier)
            console.log(result);
            return cb(d_id,v_id, v, result );
        });
    },
    ...
    _expectedValue : function (X, pow) {
        ...
        for (var i = 0; i < n; i++)
            sum += Math.pow(X[i].value, pow) / this.accuracy;
